---
title: "Amazon Store: Creation, Design & Management"
description: "I built NexLed's Amazon store from the ground up and designed the full content system that presents the brand and converts the buyer."
category: "E-Commerce"
cover:
  src: "/images/projects/project-6/storm_shots/storm_shots (1).webp"
  alt: "Amazon Store highlighted project preview"
sections:
  - component: "ProjTextCard"
    props:
      heading: "Amazon Store: Creation, Design & Management"
      labels:
        - "Figma"
        - "Photopea"
        - "Google Nano Banana"
        - "Amazon Seller Central"
        - "Affinity"
      paragraph2: >-
        BUTTONS/ACTIONS SPACE
        
  - component: "ProjTextCard"
    props:
      heading: "Context"
      paragraphs: 
        - >-
          I'm a web designer at Tec IT. But when the company decided to expand
          into B2C markets, my superior asked if I could take on the Amazon
          store, full ownership of the creation, setup, and ongoing management, with some exceptions. 
          I always been curious about e-commerce but i hadnt had the time or
          opportunity to explore it properly before and i saw a opportunity to explore some new, so i said yes.
        - >-
          I started by mapping everything out, costs, required documents, legal
          steps, account prerequisites and turned that into a structured
          report so we could move forward without surprises. What followed was a
          project that expanded well beyond what either of us originally
          anticipated.
       
  - component: "ProjTextCard"
    props:
      heading: "Research & Preparation"
      paragraphs:
       - >-
          The first phase was mostly about waiting and preparing in parallel. In the report i laid out all pieces needed, for opening the Amazon seller account, it required specific documents that unfortunately took time to
          reach me. Some of them were a new dedicated email, the legal representative's ID, proof
          of residence, amoung other. The slow internal processes meant I had to
          follow up persistently while keeping things moving elsewhere. 
       - >-
          During that time, my superior confirmed the initial product selection: four
          downlights and three LED bars, those products needed EPREL
          registration *(see EPREL project)*, packaging, and documentation, with this I
          proposed that rather than using the existing product manuals which
          were poorly structured and lacked proper step-by-step instructions  I
          design new ones from scratch, we liked the idea and agreed with my obervations, with that i started working on it.

       - >-
          The manual was a good challenge with two competing requirements:
          include every piece of necessary information, and present it in a way
          that's clear and compact enough to actually be useful. My reference
          was the IKEA manual, black and white, simple instructional drawings.
       - >-    
          I built it in Figma, designing it to anticipate the brand
          direction i had in mind, so it would hold up once the rebranding
          was done.
       - >-
          The step-by-step installation drawings were the hardest part, theses are technically complex, and I needed precise,
          clean visuals but I didn't have the time for fully manual
          illustration and my freehand drawing isn't strong enough to carry it.
          My solution was a hybrid workflow: I used Google AI image generation
          Nano Banana to generate  vector-illustration style images from reference photos and
          Photoshop composites, then manually refine the details needed. Every image had a personal pass before it went into the
          final document.
  - component: "ProjTextCard"
    props:
      heading: "Packaging and Labels"
      paragraphs: 
        - >-
          With the manuals done, I moved to packaging. I designed stock labels
          for internal use, small, black and white, consistent with the manual
          aesthetic. I also addressed the physical cardboard packaging for shipping. The
          downlights had a structural problem: the existing boxes didn't secure
          the power supply or the unit itself during transit. I designed custom
          internal supports and produced them using an
          industrial laser cutter. It was outside my field entirely, and there
          were plenty of failed cuts before the design worked. But I enjoyed
          the problem and saw it through.
        - >-
          With documentation and packaging resolved, the account was ready to
          open. I set everything up, added the necessary payment and legal
          details, and moved to what turned out to be the most involved phase:
          building the listings.
      paragraph2: ""
  - component: "ProjTextCard"
    props:
      heading: "Listings"
      paragraphs: 
        - >-
          Creating each listing required a significant amount of gathered
          information, pricing floors and ceilings, technical specifications,
          shipping box dimensions, most of which took time to arrive from other
          parts of the company. One decision key was the language
          choice. We're a Portuguese company selling on Amazon.es, a marketplace
          used by both Spanish and Portuguese buyers, but Amazon only allows
          listings in English or Spanish. 
        - >-  
          So we decided to go with Spanish, which was the right call for reach. With that in mind, I spent real time researching how to write titles and descriptions that performed well, not just translated correctly. Later, senior
          members of the team pushed to add certain information to the titles
          against my recommendations, even though i disagreed with it.
  - component: "ProjTextCard"
    props:
      heading: "Brand Registry"
      paragraphs: 
        - >-
          A significant obstacle emerged mid-process: without enrolling in
          Amazon Brand Registry, our seller account would be severely limited 
          no A+ Content, no ads, no brand storefront. Brand Registry required
          trademark registration at the EUIPO level, since the Portuguese
          national registry wasn't accepted. There was internal pushback on this
          given the cost and process, but I argued it was a necessary investment
          to use Amazon properly. Eventually it was approved, and the
          registration went through. It unblocked everything that followed.
        - >-
          While that was happening, I was also doing other work in parallel, the
          rebranding, vehicle and garage graphics, internal datasheet
          translations, and the tedious but necessary process of evaluating and
          selecting a shipping partner.
  - component: "ProjTextCard"
    props:
      heading: ""
      paragraph1: >-
        Once the brand book and visual identity were complete, I came back to
        the listings with fresh material to work with. I researched competitor
        product pages across Amazon to understand the range  and what I found
        split cleanly into two camps: brands with a coherent visual identity,
        clear sections, and purposeful copy; and brands with a cheap,
        disconnected look, redundant text, and images that had no relationship
        to the product. I used NexLed's brand guidelines and visual system to
        build something that sat firmly in the first camp.
      paragraph2: >-
        For the product photography and lifestyle images, I used Google AI
        image generation Nano Banana again with the same hybrid approach 
        generated bases, manual corrections and refinements. I designed the A+
        Content layouts, the product slides, the brand story section, and the
        storefront, across all seven products. It was long, labor-intensive
        work, but the kind where organization and attention to detail are the
        actual skill  and I was proud of the output.
  - component: "ProjTextCard"
    props:
      paragraph1: >-
        The products are live. Pricing templates and shipping rules are in
        place. I'm currently building out the ads and promotional structure
        with the goal of generating the first real sales. The full process is
        documented, and the next time we add products to Amazon, the
        groundwork is already there.
      paragraph2: ""
  - component: "ProjTextCard"
Right"
eller Central · Affinity
      paragraph2: "**Type:** E-Commerce · Content Design · Packaging · Brand Compliance"
---

